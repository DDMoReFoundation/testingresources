# Script Testing the Simulix execution
# 
# Author: mrogalski
###############################################################################
...
setwd(mdlEditorHome)
projectLoc="workspace/Simulix-Integration/models"
setwd(projectLoc)
projectLoc=getwd()

####################################
# Control flags for the Test Script
####################################
estimateLatest = FALSE
runSimulixForHandcraftedModels = TRUE
convertLatestToPharmML = TRUE
runSimulixForLatestModels = TRUE
reusePregenerated = TRUE

#
# Test Models
#
models.prev <- list(
		list(wd="DelBene_2009_oncology_in_vitro", model.mdl='DelBene_2009_oncology_in_vitro.mdl'),
		list(wd="Nock_2013_Carboplatin_PK", model.mdl='Nock_2013_Carboplatin_PK.mdl'),
		list(wd="Rocchetti_2013_oncology_TGI_antiangiogenic_combo", model.mdl='Rocchetti_2013_oncology_TGI_antiangiogenic_combo.mdl'),
		list(wd="Simeoni_2004_oncology_TGI", model.mdl='Simeoni_2004_oncology_TGI.mdl')
)
...
models.latest <- list(
		...
		list(wd="Nock_2013_Carboplatin_PK_MONOLIX_4.02", model.mdl='Nock_2013_Carboplatin_PK_MONOLIX.mdl'
				),
		...
		list(wd="Warfarin_ODE_4.02", model.mdl='Warfarin-ODE-latest.mdl')
)
...
if(estimateLatest) {
	source("EstimateLatestWithMonolix.R")
	setwd(projectLoc)
}


#models.latest = models.latest[c(1)]
if(convertLatestToPharmML) {
